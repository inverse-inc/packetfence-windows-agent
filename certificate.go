...
func getCAFingerprint(caFileBinary string) (string, error) {
	...
	if err != nil {
		...
		log.Println("Failed opening CA file: ", err)
		return "", err
	}
	// close file
	defer caFile.Close()
	...
	if _, err := io.Copy(hashSha1, caFile); err != nil {
		...
		log.Println("Failed copying CA file: ", err)
		return "", err
	}
				...
				if status, ok := exitErr.Sys().(syscall.WaitStatus); ok {
					if status.ExitStatus() == int(ERROR_CANCELED) {
						...
						if retryOrCancel == 4 {
							...
							log.Println("Failed installing certificate: ", err)
						}
					} else {
